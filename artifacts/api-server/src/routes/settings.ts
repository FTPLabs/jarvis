import { Router } from "express";
  ...
  router.patch("/", async (req, res) => {
    try {
      ...
      const map = await getAllSettings();
      const formatted = formatSettings(map);

      // Синхронизируем voiceState в voice router при изменении голосовых настроек
      try {
        const { updateVoiceStateSettings } = await import("./voice");
        updateVoiceStateSettings({
          sttEngine: formatted.sttEngine,
          ttsEngine: formatted.ttsEngine,
          wakeWord: formatted.wakeWord,
          wakeWordEnabled: formatted.wakeWordEnabled,
          language: formatted.language,
          ollamaModel: formatted.ollamaModel,
        });
      } catch {
        // voice module может быть недоступен, игнорируем
      }
