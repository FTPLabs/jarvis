...
def license_activate(req: LicenseActivateRequest):
  ...
  import re as _re
  _PERM_RE = _re.compile(r'^JARVIS-PERM-[A-Z0-9]{4}-[A-Z0-9]{4}')
  _TRIAL_RE = _re.compile(r'^JARVIS-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}')
  is_perm = bool(_PERM_RE.match(key))
  is_demo = key == "JARVIS-DEMO-TEST-2024"
  is_trial = bool(_TRIAL_RE.match(key)) or is_demo
...
{context}"""
  ...
  if learning:
      learning.record_conversation(text, answer)
